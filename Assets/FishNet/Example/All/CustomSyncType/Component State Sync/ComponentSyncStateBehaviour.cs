...
namespace FishNet.Example.ComponentStateSync
{
    public class ComponentSyncStateBehaviour : NetworkBehaviour
    {
        ...
        /// </summary>
        [SyncObject] private readonly ComponentStateSync<AMonoScript> _syncScript = new();

        private void Awake()
        {
            var ams = GetComponent<AMonoScript>();
        ...
        {
            Debug.Log(
                $"Change received on {component.GetType().Name}. New value is {nextState}. Received asServer {asServer}.");
        }
