...
namespace FishNet.Object
{
    public abstract partial class NetworkBehaviour : MonoBehaviour
    {
            ...
            Channel channel)
        {
            ...
            if (sendingClient == null)
            {
                if (_networkObjectCache.NetworkManager.CanLog(LoggingType.Error))
                    Debug.LogError(
                        $"NetworkConnection is null. Replicate {methodHash.Value} on {gameObject.name}, behaviour {GetType().Name} will not complete. Remainder of packet may become corrupt.");
                return;
            }
        ...
        {
            InternalClearReplicateCache(asServer);
        }
        ...
        public void SendReplicateRpc<T>(uint hash, List<T> replicateBuffer, int count)
        {
            ...
            var lastBufferIndex = replicateBuffer.Count - 1;
            ...
            var offset = replicateBuffer.Count - count;
            if (offset < 0)
                offset = 0;

            var channel = Channel.Unreliable;
            //Write history to methodWriter.
            var methodWriter = WriterPool.GetWriter();
        ...
        protected internal bool TransformMayChange()
        {
            ...
            var changeDistance = 0.000004f;

            var positionChanged = (transform.position - _lastMayChangePosition).sqrMagnitude > changeDistance;
            var rotationChanged = (transform.rotation.eulerAngles - _lastMayChangeRotation.eulerAngles).sqrMagnitude >
                                  changeDistance;
            var scaleChanged = (transform.localScale - _lastMayChangeScale).sqrMagnitude > changeDistance;
