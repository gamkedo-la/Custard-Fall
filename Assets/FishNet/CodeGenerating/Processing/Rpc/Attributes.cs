...
namespace FishNet.CodeGenerating.Processing.Rpc
{
    internal class Attributes
    {
        ...
        public bool HasRpcAttributes(MethodDefinition methodDef)
        {
            ...
            {
                var rt = CodegenSession.AttributeHelper.GetRpcAttributeType(customAttribute);
                if (rt != RpcType.None)
                    return true;
            }
        ...
        public List<AttributeData> GetRpcAttributes(MethodDefinition methodDef)
        {
            ...
            {
                var allRpcTypes = results.GetCombinedRpcType();
        ...
        private bool IsRpcMethodValid(MethodDefinition methodDef, RpcType rpcType)
        {
            ...
            {
                var parameterDef = methodDef.Parameters[i];
                ...
                var canSerialize =
                    CodegenSession.GeneralHelper.HasSerializerAndDeserializer(parameterDef.ParameterType, true);
                if (!canSerialize)
                {
                    CodegenSession.LogError(
