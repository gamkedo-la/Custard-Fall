...
namespace FishNet.CodeGenerating.Processing
{
    internal class NetworkBehaviourProcessor
    {
        ...
        /// </summary>
        internal List<MethodDefinition> ModifiedMethodDefinitions = new();
        ...
        /// </summary>
        private HashSet<TypeDefinition> _earlyNetworkInitializedClasses = new();

        /// <summary>
        /// Classes which have had Late NetworkInitialize methods called.
        /// </summary>
        private HashSet<TypeDefinition> _lateNetworkInitializedClasses = new();
        ...
        private MethodAttributes PROTECTED_VIRTUAL_ATTRIBUTES =
            MethodAttributes.Family | MethodAttributes.Virtual | MethodAttributes.HideBySig;

        #endregion

        internal bool Process(TypeDefinition typeDef, List<(SyncType, ProcessedSync)> allProcessedSyncs,
            Dictionary<TypeDefinition, uint> childSyncTypeCounts, Dictionary<TypeDefinition, uint> childRpcCounts)
        {
            var modified = false;
            var copyTypeDef = typeDef;
            var firstTypeDef = typeDef;
            ...
            if (!CreateOrModifyAwakeMethods(firstTypeDef))
            {
                CodegenSession.LogError(
                    $"Was unable to make Awake methods public virtual starting on type {firstTypeDef.FullName}.");
                return modified;
            }
            ...
            do
            {
                ...
                foreach (var item in nestedTds)
                    if (item.InheritsNetworkBehaviour())
                    {
                        CodegenSession.LogError(
                            $"{copyTypeDef.FullName} contains nested NetworkBehaviours. These are not supported.");
                        return modified;
                    }
            ...
            var maxAllowSyncTypes = 256;
            if (allProcessedSyncs.Count > maxAllowSyncTypes)
            {
                CodegenSession.LogError(
                    $"Found {allProcessedSyncs.Count} SyncTypes within {firstTypeDef.FullName}. The maximum number of allowed SyncTypes within type and inherited types is {maxAllowSyncTypes}. Remove SyncTypes or condense them using data containers, or a custom SyncObject.");
                return false;
            }
        ...
        internal bool NonNetworkBehaviourHasInvalidAttributes(Collection<TypeDefinition> typeDefs)
        {
            ...
            foreach (var typeDef in typeDefs)
            {
                ...
                foreach (var md in typeDef.Methods)
                    ...
                    {
                        CodegenSession.LogError(
                            $"{typeDef.FullName} has one or more RPC attributes but does not inherit from NetworkBehaviour.");
                        error = true;
                    }
                ...
                foreach (var fd in typeDef.Fields)
                    ...
                    {
                        CodegenSession.LogError(
                            $"{typeDef.FullName} has one or more SyncType attributes but does not inherit from NetworkBehaviour.");
                        error = true;
                    }
            }
        ...
        private void CallBaseAwakeMethods(TypeDefinition firstTypeDef)
        {
            ...
            do
            {
                ...
                var copyAwakeMethodDef = copyTypeDef.GetMethod(ObjectHelper.AWAKE_METHOD_NAME);
                var baseAwakeMethodDef = copyTypeDef.BaseType.CachedResolve().GetMethod(ObjectHelper.AWAKE_METHOD_NAME);
                var baseAwakeMethodRef = CodegenSession.ImportReference(baseAwakeMethodDef);

                var processor = copyAwakeMethodDef.Body.GetILProcessor();

                var alreadyHasBaseCall = false;
                ...
                foreach (var item in copyAwakeMethodDef.Body.Instructions)
                    ...
                    if (item.OpCode == OpCodes.Call || item.OpCode == OpCodes.Callvirt)
                        ...
                        {
                            var md = (MethodDefinition) item.Operand;
        ...
        private void CallNetworkInitializeFromAwake(TypeDefinition startTypeDef, bool callEarly)
        {
            ...
            var copyTypeDef = startTypeDef;
            do
            {
                var processed = callEarly ? _earlyNetworkInitializedClasses : _lateNetworkInitializedClasses;
                if (!processed.Contains(copyTypeDef))
                {
                    var methodName = callEarly
                        ? NETWORKINITIALIZE_EARLY_INTERNAL_NAME
                        : NETWORKINITIALIZE_LATE_INTERNAL_NAME;
                    var initializeMethodDef = copyTypeDef.GetMethod(methodName);
                    var initializeMethodRef = CodegenSession.ImportReference(initializeMethodDef);

                    var awakeMethodDef = copyTypeDef.GetMethod(ObjectHelper.AWAKE_METHOD_NAME);
                    var processor = awakeMethodDef.Body.GetILProcessor();
        ...
        private bool CreateOrModifyAwakeMethods(TypeDefinition typeDef)
        {
            ...
            do
            {
                var tmpMd = copyTypeDef.GetMethod(ObjectHelper.AWAKE_METHOD_NAME);
            ...
            var attributes = usePublic ? PUBLIC_VIRTUAL_ATTRIBUTES : PROTECTED_VIRTUAL_ATTRIBUTES;
            ...
            do
            {
                ...
                {
                    tmpMd = new MethodDefinition(ObjectHelper.AWAKE_METHOD_NAME, attributes,
                        copyTypeDef.Module.TypeSystem.Void);
                    copyTypeDef.Methods.Add(tmpMd);
                    var processor = tmpMd.Body.GetILProcessor();
                    processor.Emit(OpCodes.Ret);
                }
        ...
        /// </summary>
        /// <param name="typeDef"></param>
        internal void CreateFirstNetworkInitializeCall(TypeDefinition typeDef, MethodDefinition firstUserAwakeMethodDef,
            MethodDefinition firstNetworkInitializeMethodDef)
        {
            ILProcessor processor;
            //Get awake for current method.
            var thisAwakeMethodDef = typeDef.GetMethod(ObjectHelper.AWAKE_METHOD_NAME);
            var created = false;
            ...
            var networkInitializeMethodRef = typeDef.Module.ImportReference(firstNetworkInitializeMethodDef);
            ...
            {
                var baseAwakeMethodRef = typeDef.Module.ImportReference(firstUserAwakeMethodDef);
                instructions.Add(processor.Create(OpCodes.Ldarg_0)); //this.
                instructions.Add(processor.Create(OpCodes.Call, baseAwakeMethodRef));
            }
