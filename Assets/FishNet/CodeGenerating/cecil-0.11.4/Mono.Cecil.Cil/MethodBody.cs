...
namespace MonoFN.Cecil.Cil
{
    public sealed class MethodBody
    {
        ...
        private static ParameterDefinition CreateThisParameter(MethodDefinition method)
        {
            ...
            if (parameter_type.HasGenericParameters)
            {
                ...
                for (var i = 0; i < parameter_type.GenericParameters.Count; i++)
    ...
    internal sealed class VariableDefinitionCollection : Collection<VariableDefinition>
    {
        ...
        private void UpdateVariableIndices(int startIndex, int offset, VariableDefinition variableToRemove = null)
        {
            ...
            foreach (var scope in debug_info.GetScopes())
            {
                ...
                var variableDebugInfoIndexToRemove = -1;
    ...
    internal class InstructionCollection : Collection<Instruction>
    {
        ...
        private void UpdateLocalScopes(Instruction removedInstruction, Instruction existingInstruction)
        {
            ...
            var cache = new InstructionOffsetCache()
            {
                Offset = 0,
                Index = 0,
                Instruction = items[0]
        ...
        private void UpdateLocalScope(ScopeDebugInformation scope, Instruction removedInstruction,
            Instruction existingInstruction, ref InstructionOffsetCache cache)
        {
            if (scope == null)
                return;
        ...
        private InstructionOffset ResolveInstructionOffset(InstructionOffset inputOffset,
            ref InstructionOffsetCache cache)
        {
            if (inputOffset.IsResolved)
                return inputOffset;

            var offset = inputOffset.Offset;
            ...
            else
            {
                ...
                var size = cache.Offset;
                for (var i = cache.Index; i < items.Length; i++)
                {
                    cache.Index = i;
                    cache.Offset = size;
