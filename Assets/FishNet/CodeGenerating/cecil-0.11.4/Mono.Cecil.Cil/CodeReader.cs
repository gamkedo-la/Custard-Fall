    ...
    internal sealed class CodeReader : BinaryStreamReader
    {
        ...
        private void ReadAsyncMethodBody(AsyncMethodBodyDebugInformation async_method)
        {
            ...
            if (!async_method.resumes.IsNullOrEmpty())
                for (var i = 0; i < async_method.resumes.Count; i++)
        ...
        private void ReadSequencePoints()
        {
            ...
            for (var i = 0; i < symbol.sequence_points.Count; i++)
            {
        ...
        public ByteBuffer PatchRawMethodBody(MethodDefinition method, CodeWriter writer, out int code_size,
        ...
        private void PatchRawFatMethod(ByteBuffer buffer, CodeWriter writer, out int code_size,
            out MetadataToken local_var_token)
        {
            var flags = ReadUInt16();
