...
namespace MonoFN.Cecil
{
    ...
    using AssemblyRow =
        Row<AssemblyHashAlgorithm, ushort, ushort, ushort, ushort, AssemblyAttributes, uint, uint, uint>;
    using ClassLayoutRow = Row<ushort, uint, uint>;
    using ConstantRow = Row<ElementType, uint, uint>;
    using CustomAttributeRow = Row<uint, uint, uint>;
    using CustomDebugInformationRow = Row<uint, uint, uint>;
    using DeclSecurityRow = Row<SecurityAction, uint, uint>;
    using DocumentRow = Row<uint, uint, uint, uint>;
    using EventMapRow = Row<uint, uint>;
    using EventRow = Row<EventAttributes, uint, uint>;
    using ExportedTypeRow = Row<TypeAttributes, uint, uint, uint, uint>;
    using FieldLayoutRow = Row<uint, uint>;
    using FieldMarshalRow = Row<uint, uint>;
    using FieldRow = Row<FieldAttributes, uint, uint>;
    using FieldRVARow = Row<uint, uint>;
    using FileRow = Row<FileAttributes, uint, uint>;
    using GenericParamConstraintRow = Row<uint, uint>;
    using GenericParamRow = Row<ushort, GenericParameterAttributes, uint, uint>;
    using ImplMapRow = Row<PInvokeAttributes, uint, uint, uint>;
    using ImportScopeRow = Row<uint, uint>;
    using InterfaceImplRow = Row<uint, uint>;
    using LocalConstantRow = Row<uint, uint>;
    using LocalScopeRow = Row<uint, uint, uint, uint, uint, uint>;
    using LocalVariableRow = Row<VariableAttributes, ushort, uint>;
    using ManifestResourceRow = Row<uint, ManifestResourceAttributes, uint, uint>;
    using MemberRefRow = Row<uint, uint, uint>;
    using MethodDebugInformationRow = Row<uint, uint>;
    using MethodImplRow = Row<uint, uint, uint>;
    using MethodRow = Row<uint, MethodImplAttributes, MethodAttributes, uint, uint, uint>;
    using MethodSemanticsRow = Row<MethodSemanticsAttributes, uint, uint>;
    using MethodSpecRow = Row<uint, uint>;
    using ModuleRow = Row<uint, uint>;
    using NestedClassRow = Row<uint, uint>;
    using ParamRow = Row<ParameterAttributes, ushort, uint>;
    using PropertyMapRow = Row<uint, uint>;
    using PropertyRow = Row<PropertyAttributes, uint, uint>;
    using StateMachineMethodRow = Row<uint, uint>;
    using TypeDefRow = Row<TypeAttributes, uint, uint, uint, uint, uint>;
    using TypeRefRow = Row<uint, uint, uint>;
    ...
    internal static class ModuleWriter
    {
        ...
        private static void Write(ModuleDefinition module, Disposable<Stream> stream, WriterParameters parameters)
        {
            ...
            if (module.HasImage && module.ReadingMode == ReadingMode.Deferred)
            {
                ...
                immediate_reader.ReadModule(module, false);
        ...
        private static ISymbolWriter GetSymbolWriter(ModuleDefinition module, string fq_name,
            ISymbolWriterProvider symbol_writer_provider, WriterParameters parameters)
        {
    ...
    internal sealed class MetadataBuilder
    {
        ...
        internal uint type_rid = 1;
        internal uint field_rid = 1;
        internal uint method_rid = 1;
        internal uint param_rid = 1;
        internal uint property_rid = 1;
        internal uint event_rid = 1;
        internal uint local_variable_rid = 1;
        internal uint local_constant_rid = 1;
        ...
        public MetadataBuilder(ModuleDefinition module, string fq_name, uint timestamp,
            ISymbolWriterProvider symbol_writer_provider)
        {
            ...
            import_scope_map = new Dictionary<ImportScopeRow, MetadataToken>(row_equality_comparer);
        }
        ...
        private void AddExportedTypes()
        {
            ...
            for (var i = 0; i < exported_types.Count; i++)
            {
        ...
        private void AttachNestedTypesToken(TypeDefinition type)
        {
            ...
            for (var i = 0; i < nested_types.Count; i++)
        ...
        private void AddCustomAttributes(ICustomAttributeProvider owner)
        {
            ...
            for (var i = 0; i < custom_attributes.Count; i++)
            {
        ...
        {
            state_machine_method_table.AddRow(new StateMachineMethodRow(method_info.Method.MetadataToken.RID,
                method_info.StateMachineKickOffMethod.MetadataToken.RID));
        }
        ...
        private void AddLocalVariables(ScopeDebugInformation scope)
        {
            for (var i = 0; i < scope.Variables.Count; i++)
            {
                ...
                local_variable_table.AddRow(new LocalVariableRow(variable.Attributes, (ushort) variable.Index,
                    GetStringIndex(variable.Name)));
        ...
        {
            for (var i = 0; i < scope.Constants.Count; i++)
            {
                var constant = scope.Constants[i];
                local_constant_table.AddRow(new LocalConstantRow(GetStringIndex(constant.Name),
                    GetBlobIndex(GetConstantSignature(constant))));
        ...
        public void AddCustomDebugInformations(ICustomDebugInformationProvider provider)
        {
            ...
            for (var i = 0; i < custom_infos.Count; i++)
            {
        ...
        private void AddStateMachineScopeDebugInformation(ICustomDebugInformationProvider provider,
        ...
        private void AddAsyncMethodBodyDebugInformation(ICustomDebugInformationProvider provider,
            AsyncMethodBodyDebugInformation async_method)
        {
            var signature = CreateSignatureWriter();
            signature.WriteUInt32((uint) async_method.catch_handler.Offset + 1);

            if (!async_method.yields.IsNullOrEmpty())
                for (var i = 0; i < async_method.yields.Count; i++)
                {
        ...
        private void AddEmbeddedSourceDebugInformation(ICustomDebugInformationProvider provider,
            EmbeddedSourceDebugInformation embedded_source)
        {
            var signature = CreateSignatureWriter();
            ...
            if (embedded_source.compress)
            {
                ...
                using (var compress_stream = new DeflateStream(content_stream, CompressionMode.Compress, true))
                {
        ...
        private void AddSourceLinkDebugInformation(ICustomDebugInformationProvider provider,
            SourceLinkDebugInformation source_link)
        {
            var signature = CreateSignatureWriter();
        ...
        private void AddCustomDebugInformation(ICustomDebugInformationProvider provider,
            CustomDebugInformation custom_info, SignatureWriter signature)
        {
            AddCustomDebugInformation(provider, custom_info, GetBlobIndex(signature));
        }

        private void AddCustomDebugInformation(ICustomDebugInformationProvider provider,
            CustomDebugInformation custom_info, uint blob_index)
        {
        ...
        private uint AddImportScope(ImportDebugInformation import)
        {
            ...
            if (import.HasTargets)
            {
                ...
                for (var i = 0; i < import.Targets.Count; i++)
    ...
    internal sealed class SignatureWriter : ByteBuffer
    {
        ...
        private void WriteCustomAttributeTypeValue(TypeReference value)
        {
            ...
            {
                var outermostDeclaringType = typeDefinition;
        ...
        {
            for (var i = 0; i < named_arguments.Count; i++)
        ...
        public void WriteSequencePoints(MethodDebugInformation info)
        {
            ...
            for (var i = 0; i < info.SequencePoints.Count; i++)
            {
    ...
    internal static partial class Mixin
    {
        public static bool TryGetUniqueDocument(this MethodDebugInformation info, out Document document)
        {
            ...
            for (var i = 1; i < info.SequencePoints.Count; i++)
            {
                var sequence_point = info.SequencePoints[i];
