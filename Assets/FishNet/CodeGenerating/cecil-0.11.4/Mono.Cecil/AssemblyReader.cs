...
namespace MonoFN.Cecil
{
    ...
    internal sealed class ImmediateModuleReader : ModuleReader
    {
        ...
        private void ReadSecurityDeclarations(ISecurityDeclarationProvider provider)
        {
            ...
            for (var i = 0; i < security_declarations.Count; i++)
            {
    ...
    internal sealed class MetadataReader : ByteBuffer
    {
        ...
        private void InitializeAssemblyReferences()
        {
            ...
            var length = MoveTo(Table.AssemblyRef);
        ...
        private void InitializeModuleReferences()
        {
            ...
            var length = MoveTo(Table.ModuleRef);
        ...
        {
            var length = MoveTo(Table.ManifestResource);
        ...
        private void InitializeTypeDefinitions()
        {
            ...
            var length = MoveTo(Table.TypeDef);
        ...
        private static Collection<TValue> AddMapping<TKey, TValue>(Dictionary<TKey, Collection<TValue>> cache, TKey key,
            TValue value)
        {
            Collection<TValue> mapped;
            if (!cache.TryGetValue(key, out mapped)) mapped = new Collection<TValue>();
            mapped.Add(value);
            return mapped;
        }
        ...
        private void InitializeTypeLayouts()
        {
            ...
            var length = MoveTo(Table.ClassLayout);
        ...
        private void InitializeInterfaces()
        {
            ...
            var length = MoveTo(Table.InterfaceImpl);
        ...
        private void InitializeFieldRVAs()
        {
            ...
            var length = MoveTo(Table.FieldRVA);
        ...
        private void InitializeFieldLayouts()
        {
            ...
            var length = MoveTo(Table.FieldLayout);
        ...
        private void InitializeEvents()
        {
            ...
            var length = MoveTo(Table.EventMap);
            ...
            for (uint i = 1; i <= length; i++)
            {
                ...
                var events_range = ReadListRange(i, Table.EventMap, Table.Event);
        ...
        private void InitializeProperties()
        {
            ...
            var length = MoveTo(Table.PropertyMap);
        ...
        private void InitializeMethodSemantics()
        {
            ...
            var length = MoveTo(Table.MethodSemantics);
        ...
        private void InitializePInvokes()
        {
            ...
            var length = MoveTo(Table.ImplMap);
        ...
        private void InitializeDocuments()
        {
            ...
            var length = MoveTo(Table.Document);
        ...
        private void InitializeLocalScopes()
        {
            ...
            var length = MoveTo(Table.LocalScope);
            ...
            for (uint i = 1; i <= length; i++)
            {
                ...
                metadata.SetLocalScopes(method,
                    AddMapping(metadata.LocalScopes, method,
                        new Row<uint, Range, Range, uint, uint, uint>(import, variables, constants, scope_start,
                            scope_length, i)));
            }
        }
        ...
        private VariableDebugInformation ReadLocalVariable(uint rid)
        {
            ...
            var variable = new VariableDebugInformation(index, name)
                {Attributes = attributes, token = new MetadataToken(TokenType.LocalVariable, rid)};
        ...
        private ConstantDebugInformation ReadLocalConstant(uint rid)
        {
            ...
            else if (type.IsTypeOf("System", "Decimal"))
            {
                ...
                value = new decimal(signature.ReadInt32(), signature.ReadInt32(), signature.ReadInt32(),
                    (b & 0x80) != 0, (byte) (b & 0x7f));
            }
            else if (type.IsTypeOf("System", "DateTime"))
            {
                value = new DateTime(signature.ReadInt64());
            }
            else if (type.etype == ElementType.Object || type.etype == ElementType.None ||
                     type.etype == ElementType.Class || type.etype == ElementType.Array ||
                     type.etype == ElementType.GenericInst)
            {
                value = null;
            }
            else
            ...
            var constant = new ConstantDebugInformation(name, type, value)
        ...
        public Row<byte[], bool> ReadEmbeddedSourceDebugInformation(uint index)
        {
            ...
            else if (format > 0)
            {
                ...
                var decompressed_document =
                    new byte [format]; // if positive, format is the decompressed length of the document
                var decompressed_stream = new MemoryStream(decompressed_document);

                using (var deflate_stream = new DeflateStream(compressed_stream, CompressionMode.Decompress, true))
                {
    ...
    internal sealed class SignatureReader : ByteBuffer
    {
        ...
        private static void CheckGenericContext(IGenericParameterProvider owner, int index)
        {
            ...
            for (var i = owner_parameters.Count; i <= index; i++)
        ...
        private ArrayType ReadArrayTypeSignature()
        {
            ...
            for (var i = 0; i < low_bounds.Length; i++)
