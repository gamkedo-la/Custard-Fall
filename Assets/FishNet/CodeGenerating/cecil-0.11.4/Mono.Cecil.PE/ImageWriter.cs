...
namespace MonoFN.Cecil.PE
{
    internal sealed class ImageWriter : BinaryStreamWriter
    {
        ...
        private ImageWriter(ModuleDefinition module, string runtime_version, MetadataBuilder metadata,
            Disposable<Stream> stream, bool metadataOnly = false)
            : base(stream.value)
        {
            this.module = module;
        ...
        private void GetWin32Resources()
        {
            ...
            var win32_resources_directory = module.Image.Win32Resources;
            var size = win32_resources_directory.Size;

            if (size > 0)
                win32_resources = module.Image.GetReaderAt(win32_resources_directory.VirtualAddress, size,
                    (s, reader) => new ByteBuffer(reader.ReadBytes((int) s)));
        }

        public static ImageWriter CreateWriter(ModuleDefinition module, MetadataBuilder metadata,
        ...
        public static ImageWriter CreateDebugWriter(ModuleDefinition module, MetadataBuilder metadata,
            Disposable<Stream> stream)
        {
            var writer = new ImageWriter(module, "PDB v1.0", metadata, stream, true);
        ...
        private void WriteSection(Section section, uint characteristics)
        {
            ...
            for (var i = 0; i < sect_name.Length; i++)
        ...
        public void WriteMetadataHeader()
        {
            ...
            var offset = text_map.GetRVA(TextSegment.TableHeap) - text_map.GetRVA(TextSegment.MetadataHeader);
        ...
        private void BuildTextMap()
        {
            ...
            var import_dir_rva = map.GetNextRVA(TextSegment.DebugDirectory);
            var import_hnt_rva = import_dir_rva + 48u;
            import_hnt_rva = (import_hnt_rva + 15u) & ~15u;
            var import_dir_len = import_hnt_rva - import_dir_rva + 27u;

            var startup_stub_rva = import_dir_rva + import_dir_len;
