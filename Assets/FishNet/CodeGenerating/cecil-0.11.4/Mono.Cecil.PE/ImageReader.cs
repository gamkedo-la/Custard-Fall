...
namespace MonoFN.Cecil.PE
{
    internal sealed class ImageReader : BinaryStreamReader
    {
        ...
        private void ReadImage()
        {
            ...
            // Characteristics		2
            var characteristics = ReadUInt16();
        ...
        private void ReadMetadataStream(Section section)
        {
            // Offset		4
            var offset =
                metadata.VirtualAddress - section.VirtualAddress + ReadUInt32(); // relative to the section start
        ...
        private void ReadTableHeap()
        {
            ...
            if (image.PdbHeap != null)
                for (var i = 0; i < Mixin.TableCount; i++)
                {
        ...
        {
            var offset =
                (uint) BaseStream.Position - table_heap_offset - image.MetadataSection.PointerToRawData; // header

            var stridx_size = image.StringHeap != null ? image.StringHeap.IndexSize : 2;
            var guididx_size = image.GuidHeap != null ? image.GuidHeap.IndexSize : 2;
            var blobidx_size = image.BlobHeap != null ? image.BlobHeap.IndexSize : 2;
