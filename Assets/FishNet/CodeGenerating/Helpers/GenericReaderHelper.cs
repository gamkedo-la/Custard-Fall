...
namespace FishNet.CodeGenerating.Helping
{
    internal class GenericReaderHelper
    {
        ...
        internal const MethodAttributes INITIALIZEONCE_METHOD_ATTRIBUTES =
            GenericWriterHelper.INITIALIZEONCE_METHOD_ATTRIBUTES;
            ...
            * generated. */
            if (_generatedReaderWriterClassTypeDef == null)
                _generatedReaderWriterClassTypeDef = CodegenSession.GeneralHelper.GetOrCreateClass(out _,
                    ReaderGenerator.GENERATED_TYPE_ATTRIBUTES, ReaderGenerator.GENERATED_READERS_CLASS_NAME, null);
            ...
            var dataTypeRef = readMethodRef.ReturnType;
            ...
            var isAutoPacked = CodegenSession.ReaderHelper.IsAutoPackedType(dataTypeRef);

            //Generate for autopacktype.
            if (isAutoPacked)
            {
                functionGenericInstance =
                    _functionT3TypeRef.MakeGenericInstanceType(_readerTypeRef, _autoPackTypeRef, dataTypeRef);
                functionConstructorInstanceMethodRef =
                    _functionT3ConstructorMethodRef.MakeHostInstanceGeneric(functionGenericInstance);
            }
            //Not autopacked.
            else
            {
                ...
                functionConstructorInstanceMethodRef =
                    _functionT2ConstructorMethodRef.MakeHostInstanceGeneric(functionGenericInstance);
            }

            processor.Emit(OpCodes.Newobj, functionConstructorInstanceMethodRef);

            //Call delegate to GeneratedReader<T>.Read
            var genericInstance = _genericReaderTypeRef.MakeGenericInstanceType(dataTypeRef);
            var genericReaderMethodRef = isAutoPacked
