...
namespace FishNet.CodeGenerating.Helping
{
    internal class WriterHelper
    {
        ...
        internal readonly Dictionary<TypeReference, MethodReference> _instancedWriterMethods =
            new(new TypeReferenceComparer());

        private readonly Dictionary<TypeReference, MethodReference> _staticWriterMethods =
            new(new TypeReferenceComparer());

        private HashSet<TypeReference> _autoPackedMethods = new(new TypeReferenceComparer());
        ...
        internal bool ImportReferences()
        {
            ...
            foreach (var methodInfo in pooledWriterType.GetMethods())
            {
                ...
                * there is a pack option. */
                ...
                var typeRef = CodegenSession.ImportReference(parameterInfos[0].ParameterType);
                /* If here all checks pass. */
                var methodRef = CodegenSession.ImportReference(methodInfo);
                AddWriterMethod(typeRef, methodRef, true, true);
                if (autoPackMethod)
                    _autoPackedMethods.Add(typeRef);
            }

            var writerExtensionsType = typeof(WriterExtensions);
            foreach (var methodInfo in writerExtensionsType.GetMethods())
            {
                 ...
                 * The first parameter will always be the type written. */
                var typeRef = CodegenSession.ImportReference(parameterInfos[1].ParameterType);
                /* If here all checks pass. */
                var methodRef = CodegenSession.ImportReference(methodInfo);
        ...
        internal bool CreateGenericDelegates()
        {
            ...
            foreach (var item in _staticWriterMethods)
                if (FishNetILPP.CODEGEN_THIS_NAMESPACE.Length == 0 ||
                    item.Key.FullName.Contains(FishNetILPP.CODEGEN_THIS_NAMESPACE))
                {
        ...
        internal bool HasSerializer(TypeReference typeRef, bool createMissing)
        {
            ...
            if (!result && createMissing)
                ...
                {
                    var methodRef = CodegenSession.WriterGenerator.CreateWriter(typeRef);
        ...
        {
            _instancedWriterMethods.TryGetValue(typeRef, out var methodRef);
            return methodRef;
        }
        ...
        {
            _staticWriterMethods.TryGetValue(typeRef, out var methodRef);
            return methodRef;
        }
        ...
        internal MethodReference GetOrCreateFavoredWriteMethodReference(TypeReference typeRef, bool favorInstanced)
        {
            ...
            var writeMethodRef = GetFavoredWriteMethodReference(typeRef, favorInstanced);
        ...
        {
            var dict = instanced ? _instancedWriterMethods : _staticWriterMethods;
        ...
        internal VariableDefinition CreatePooledWriter(MethodDefinition methodDef)
        {
            ...
            var insts = CreatePooledWriter(methodDef, out resultVd);
        ...
        internal void CreateWritePackedWhole(ILProcessor processor, ParameterDefinition writerParameterDef, int value)
        {
            ...
            var intVariableDef = CodegenSession.GeneralHelper.CreateVariable(processor.Body.Method, typeof(int));
        ...
        {
            var writeBoolMethodRef =
                GetFavoredWriteMethodReference(CodegenSession.GeneralHelper.GetTypeReference(typeof(bool)), true);
            processor.Emit(OpCodes.Ldarg, writerParameterDef);
        ...
        internal List<Instruction> CreateWriteInstructions(MethodDefinition methodDef, object pooledWriterDef,
            ParameterDefinition valueParameterDef, MethodReference writeMethodRef)
        {
            var insts = new List<Instruction>();
            var processor = methodDef.Body.GetILProcessor();
            ...
            if (writeMethodRef != null)
            {
                ...
                {
                    var packType = CodegenSession.GeneralHelper.GetDefaultAutoPackType(valueParameterDef.ParameterType);
        ...
        internal void CreateWrite(ILProcessor processor, ParameterDefinition writerPd, FieldDefinition fieldDef,
            MethodReference writeMr)
        {
            if (writeMr != null)
            {
                var fieldRef = CodegenSession.GeneralHelper.GetFieldReference(fieldDef);
                ...
                {
                    var packType = CodegenSession.GeneralHelper.GetDefaultAutoPackType(fieldDef.FieldType);
