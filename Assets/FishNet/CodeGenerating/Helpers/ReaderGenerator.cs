...
namespace FishNet.CodeGenerating.Helping
{
    internal class ReaderGenerator
    {
        ...
        internal MethodReference CreateReader(TypeReference objectTr)
        {
            ...
            var serializerType = GeneratorHelper.GetSerializerType(objectTr, false, out objectTypeDef);
        ...
        private MethodReference CreateEnumReaderMethodDefinition(TypeReference objectTr)
        {
            ...
            var underlyingTypeRef = objectTr.CachedResolve().GetEnumUnderlyingTypeReference();
            //Get read method for underlying type.
            var readMethodRef =
                CodegenSession.ReaderHelper.GetOrCreateFavoredReadMethodReference(underlyingTypeRef, true);
        ...
        private MethodReference GetNetworkBehaviourReaderMethodReference(TypeReference objectTr)
        {
            ...
            var processor = createdReaderMd.Body.GetILProcessor();
            var networkBehaviourTypeRef = CodegenSession.GeneralHelper.GetTypeReference(typeof(NetworkBehaviour));

            processor.Emit(OpCodes.Ldarg_0);
            processor.Emit(OpCodes.Call,
                CodegenSession.ReaderHelper.GetFavoredReadMethodReference(networkBehaviourTypeRef, true));
        ...
        private MethodReference CreateArrayReaderMethodReference(TypeReference objectTr)
        {
            ...
            var readMethodRef = CodegenSession.ReaderHelper.GetOrCreateFavoredReadMethodReference(elementTypeRef, true);
            if (readMethodRef == null)
                return null;
            ...
            var readerParameterDef = createdReaderMd.Parameters[0];
            var sizeVariableDef = CodegenSession.GeneralHelper.CreateVariable(createdReaderMd, typeof(int));
            ...
            var collectionVariableDef = CodegenSession.GeneralHelper.CreateVariable(createdReaderMd, objectTr);
            ...
            var loopIndex = CodegenSession.GeneralHelper.CreateVariable(createdReaderMd, typeof(int));
            ...
            {
                var packType = CodegenSession.GeneralHelper.GetDefaultAutoPackType(elementTypeRef);
        ...
        private MethodReference CreateDictionaryReaderMethodReference(TypeReference objectTr)
        {
            ...
            /* Try to get instanced first for collection element type, if it doesn't exist then try to
             * get/or make a one. */
            var keyWriteMr = CodegenSession.ReaderHelper.GetOrCreateFavoredReadMethodReference(keyTr, true);
            var valueWriteMr = CodegenSession.ReaderHelper.GetOrCreateFavoredReadMethodReference(valueTr, true);
            ...
            var genericInstanceMethod =
                CodegenSession.ReaderHelper.Reader_ReadDictionary_MethodRef.MakeGenericMethod(new TypeReference[]
                    {keyTr, valueTr});
        ...
        private MethodReference CreateListReaderMethodReference(TypeReference objectTr)
        {
            var genericInstance = (GenericInstanceType) objectTr;
            CodegenSession.ImportReference(genericInstance);
            var elementTypeRef = genericInstance.GenericArguments[0];
            ...
            var constructorMd = objectTr.CachedResolve().GetConstructor(new Type[] {typeof(int)});
            var constructorMr = constructorMd.MakeHostInstanceGeneric(genericInstance);
            //Find add method for list.
            MethodReference lstAddMd = objectTr.CachedResolve().GetMethod("Add");
            var lstAddMr = lstAddMd.MakeHostInstanceGeneric(genericInstance);
            ...
            var collectionVariableDef = CodegenSession.GeneralHelper.CreateVariable(createdReaderMd, genericInstance);
            processor.Emit(OpCodes.Ldloc, sizeVariableDef);
        ...
        private MethodReference CreateNullableReaderMethodReference(TypeReference objectTr)
        {
            ...
            var objectCtorMd = objectTr.GetConstructor(1);
            ...
            var valueReaderMr = CodegenSession.ReaderHelper.GetOrCreateFavoredReadMethodReference(valueTr, true);
            if (valueReaderMr == null)
                return null;

            var objectTd = objectTr.CachedResolve();
            var createdReaderMd = CreateStaticReaderStubMethodDefinition(objectTr);
            AddToStaticReaders(objectTr, createdReaderMd);
            ...
            var resultVd = CodegenSession.GeneralHelper.CreateVariable(createdReaderMd, objectTr);

            //Read if null into boolean.
            var nullBoolVd = createdReaderMd.CreateVariable(typeof(bool));
            ...
            var initMr = objectCtorMd.MakeHostInstanceGeneric(objectGit);
            processor.Emit(OpCodes.Ldarg, readerPd);
            //If an auto pack method then insert default value.
            if (CodegenSession.ReaderHelper.IsAutoPackedType(valueTr))
            {
                var packType = CodegenSession.GeneralHelper.GetDefaultAutoPackType(valueTr);
                processor.Emit(OpCodes.Ldc_I4, (int) packType);
            }
        ...
        private MethodReference CreateClassOrStructReaderMethodReference(TypeReference objectTr)
        {
            ...
            var objectTypeDef = objectTr.CachedResolve();
            var processor = createdReaderMd.Body.GetILProcessor();

            var readerParameterDef = createdReaderMd.Parameters[0];
            // create local for return value
            var objectVariableDef = CodegenSession.GeneralHelper.CreateVariable(createdReaderMd, objectTr);

            //If not a value type create a return null check.
            if (!objectTypeDef.IsValueType)
            {
                var nullVariableDef = CodegenSession.GeneralHelper.CreateVariable(createdReaderMd, typeof(bool));
                //Load packed whole value into sizeVariableDef, exit if null indicator.
        ...
        /// </summary>  
        private bool ReadFields(MethodDefinition methodDef, ParameterDefinition readerPd, VariableDefinition objectVd,
            TypeReference objectTr)
        {
            ...
            foreach (var fieldDef in objectTr.FindAllPublicFields())
            {
                var fieldRef = CodegenSession.ImportReference(fieldDef);
                var typeRef = CodegenSession.ImportReference(fieldRef.FieldType);
                var readMethodRef = CodegenSession.ReaderHelper.GetOrCreateFavoredReadMethodReference(typeRef, true);
        ...
        {
            var methodName = $"{READ_PREFIX}{objectTypeRef.FullName}{nameExtension}s";
            // create new reader for this type
            var readerTypeDef = CodegenSession.GeneralHelper.GetOrCreateClass(out _, GENERATED_TYPE_ATTRIBUTES,
            ...
            CodegenSession.GeneralHelper.CreateParameter(readerMethodDef, CodegenSession.ReaderHelper.Reader_TypeRef,
                "reader");
