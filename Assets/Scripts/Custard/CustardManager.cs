...
public class CustardManager : MonoBehaviour
{
    ...
    private bool IsInitialWorldCustard(byte x, byte y)
    {
        ...
        return x == 0 || y == 0 || x == WorldCells.BlocksWidth - 1 || y == WorldCells.BlocksHeight - 1 || (x == 55 && y == 55) ||
    ...
    private void SimulateCustard()
    {
        ...
        {
            var custardAreaOfEffect = GetLocalNeighborhood(coords, (i,j)=>custardState.GetCurrentCustardLevelAt(i,j));
    ...
    {
        if ((pivotCoords.X == 0 && x == -1) || (pivotCoords.X == WorldCells.BlocksWidth - 1 && x == 1) || (pivotCoords.Y == 0 && y == -1) ||
